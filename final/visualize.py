...
def bug_visulize(robot_position, goal, map_size, obstacle):
    fig, ax = plt.subplots()
    ax.cla()
    ax.scatter(np.array(goal)[0, 1], np.array(goal)[0, 0], marker='s',s=200,color='w',edgecolor='b')
    ax.scatter(np.array(goal)[1, 1], np.array(goal)[1, 0], marker='s', s=200, color='w', edgecolor='g')
    ...
    for j in range(6):
        ax.annotate(str(j), xy=(goal[j][1]-0.1, goal[j][0]-0.1))
    ax.scatter(np.array(robot_position)[0, 1], np.array(robot_position)[0, 0], marker='s',s=100,color='b')
    ax.scatter(np.array(robot_position)[1, 1], np.array(robot_position)[1, 0], marker='s',s=100,color='g')
    ax.scatter(np.array(robot_position)[2, 1], np.array(robot_position)[2, 0], marker='s',s=100,color='m')
    ax.scatter(np.array(robot_position)[3, 1], np.array(robot_position)[3, 0], marker='s',s=100,color='y')
    ax.scatter(np.array(robot_position)[4, 1], np.array(robot_position)[4, 0], marker='s',s=100,color='c')
    ax.scatter(np.array(robot_position)[5, 1], np.array(robot_position)[5, 0], marker='s',s=100,color='r')
    for j in range(6):
        ax.annotate(str(j), xy=(robot_position[j][1]-0.1, robot_position[j][0]-0.1))
    if obstacle != []:
        ax.scatter(np.array(obstacle)[:, 1], np.array(obstacle)[:, 0], color='k', marker='s',s=600)
    plt.axis([-1, map_size[1], -1, map_size[0]])
    # ax.legend()
...
def obstacle_visualize(robot_position, goal, map_size, obstacle,narrow):
    fig, ax = plt.subplots()
    ax.cla()
    ax.scatter(np.array(goal)[0, 1], np.array(goal)[0, 0], marker='s',s=200,color='w',edgecolor='b')
    ax.scatter(np.array(goal)[1, 1], np.array(goal)[1, 0], marker='s', s=200, color='w', edgecolor='g')
    ...
    if obstacle != []:
        ax.scatter(np.array(obstacle)[:, 1], np.array(obstacle)[:, 0], color='k', marker='s',s=600)
        ax.scatter(np.array(narrow)[:, 1], np.array(narrow)[:, 0], color='gray', marker='s', s=600)
    plt.axis([-1, map_size[1], -1, map_size[0]])
    # ax.legend()
    ax.set_aspect('equal','box')
    ax.grid()
...
if __name__ == '__main__':
    ...
    # robot_position = [[0, 6], [2, 6], [0, 7],[2, 8], [1, 6], [1, 7]]
    # bug_visulize(robot_position, agent_goal, map_size, obstacle)
    ...
    robot_position = [[3,5],[2,5],[3,6],[3,3],[2,4],[3,4]]
    bug_visulize(robot_position, agent_goal, map_size, obstacle)
    ...
    # original_path,original_path_no_start = original_path(map_size, obstacle, agent_start, agent_goal)
    # robot_position = final_path(agent_start, agent_goal, original_path, original_path_no_start)
    robot_position = agent_start

    obstacle_visualize(robot_position, agent_goal, map_size, obstacle,narrow_1)

    obstacle_visualize(robot_position, agent_goal, map_size, obstacle, narrow_2)